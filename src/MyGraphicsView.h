...
// constexpr static int MAX_NODECNT = 1 * (1 - std::pow(4, MAX_DEPTH)) / (1 -
// 4); constexpr static int MAX_LEAFCNT = std::pow(4, MAX_DEPTH - 1); constexpr
// static int MAX_NODEPOOL = MAX_NODECNT; constexpr static int MAX_LEAFPOOL =
// MAX_LEAFCNT;
...
struct QuadtreeNode {
  ...
  QuadtreeNode(double _x, double _y, double _w, double _h, int _depth) // NOLINT
      : x{_x}, y{_y}, w{_w}, h{_h}, depth{_depth} {}
  ~QuadtreeNode() = default;

  void divide() {
    double neww{w / 2}, newh{h / 2};
    s[0] = new QuadtreeNode{x + neww, y, neww, newh, depth + 1};
    s[1] = new QuadtreeNode{x, y, neww, newh, depth + 1};
    s[2] = new QuadtreeNode{x, y + newh, neww, newh, depth + 1};
    s[3] = new QuadtreeNode{x + neww, y + newh, neww, newh, depth + 1};
  }
  ...
  std::vector<int> getManagerIndex(MyCollisionBall *obj) {
    ...
    bool up{objy + objr > midy}, down{objy - objr < midy},
        left{objx - objr < midx}, right{objx + objr > midx};
    if (up) {
      if (left)
...
protected:
  MyCollisionBall *addNewBall(double x, double y) {
    ...
    if (ballFreeTop && ballFreeTop < MAX_BALLPOOL) {
      ...
      addr = &(ballPool[ballPoolTop++]); // NOLINT
    }
...
public:
  MyGraphicsView(QWidget *parent) : QGraphicsView{parent} {
    ...
    ballPool = static_cast<MyCollisionBall *>(operator new[](
        sizeof(MyCollisionBall) * MAX_BALLPOOL, std::nothrow));
    ballFreeStack = static_cast<MyCollisionBall **>(operator new[](
        sizeof(MyCollisionBall *) * MAX_BALLPOOL, std::nothrow));
  ...
  ~MyGraphicsView() override {
    for (int i = 0; i < ballPoolTop; i++) {
      ballPool[i].~MyCollisionBall(); // NOLINT
    }
    operator delete[](ballPool, sizeof(MyCollisionBall) * MAX_BALLPOOL);
    operator delete[](ballFreeStack,                             // NOLINT
                      sizeof(MyCollisionBall *) * MAX_BALLPOOL); // NOLINT
  }
  ...
  inline bool mouseBallTracking(bool status) {
    return this->m_mouseBallTracking;
  }
...
protected:
  ...
  void paintEvent(QPaintEvent *event) override {
    ...
    painter.drawText(
        20, 20, "Collision Times " + QString::number(this->collisionTimes));
    painter.drawText(20, 40,
                     "Compare Times " + QString::number(this->comparedTimes));
  }
  ...
  void mousePressEvent(QMouseEvent *event) override {
    ...
    if (event->buttons() & Qt::MouseButton::RightButton) {
      if (this->m_mouseAddBall) {
        ...
        const auto toScene{mapToScene(
            (int)(std::min(std::max(mousePos.x(), (double)newBallRadius),
                           maxX - (double)newBallRadius)),
            int(std::min(std::max(mousePos.y(), (double)newBallRadius),
                         maxY - (double)newBallRadius)))};
  ...
  void mouseMoveEvent(QMouseEvent *event) override {
    ...
    if (this->m_mouseBallTracking &&
      ...
      auto toScene{mapToScene(
          (int)(std::min(std::max(mousePos.x(), mouseBall->getRadius()),
                         maxX - mouseBall->getRadius())),
          (int)(std::min(std::max(mousePos.y(), mouseBall->getRadius()),
                         maxY - mouseBall->getRadius())))};

      // Eigen::Vector2d lastPos{ mouseBall->getPosV() };
      // Eigen::Vector2d newPos{ toScene.x(), toScene.y() };
      // Eigen::Vector2d newVelo{ (newPos - lastPos) / (this->clock->interval()
      // / 1000.0) };
  ...
  void brutalCollisionCheck() {
    ...
    for (auto i : items) {
      (static_cast<MyCollisionBall *>(i))
          ->updatePosByVelo(this->clock->interval());
    }
  }
